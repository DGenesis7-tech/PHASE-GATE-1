def age_difference(fathers_age, sons_age):
	if not isinstance(fathers_age, (int, float)):
		raise ValueError("fathers age must be a number with not floating value")
	
	if not isinstance(sons_age, (int, float)):
		raise ValueError("sons age must be a number with not floating value")
